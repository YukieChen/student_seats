/**
 * 結果分析器
 * 負責分析分配結果、條件滿足情況、失敗原因等
 */
const { Logger } = require('./Logger.js');

class ResultAnalyzer {
	constructor(options = {}) {
		this.logger = new Logger('ResultAnalyzer');
		this.options = {
	...
	generateSuggestions(analysisResult) {
		...
		if (analysisResult.failureType === 'TIMEOUT') {
			suggestions.push({
				priority: this.ANALYSIS_CONFIG.SUGGESTION_PRIORITIES.HIGH,
		...
		if (analysisResult.failureType === 'CONFLICT') {
			suggestions.push({
				priority: this.ANALYSIS_CONFIG.SUGGESTION_PRIORITIES.MEDIUM,
				category: 'LOGIC',
				title: '檢查條件設定',
	...
	prioritizeSuggestions(suggestions) {
		const priorityOrder = [
			this.ANALYSIS_CONFIG.SUGGESTION_PRIORITIES.CRITICAL,
			this.ANALYSIS_CONFIG.SUGGESTION_PRIORITIES.HIGH,
			this.ANALYSIS_CONFIG.SUGGESTION_PRIORITIES.MEDIUM,
			this.ANALYSIS_CONFIG.SUGGESTION_PRIORITIES.LOW
		];
