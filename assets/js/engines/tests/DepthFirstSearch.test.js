...
function runTests() {
	...
	test('深度優先搜索基本功能測試', async () => {
		...
		if (!result.success) throw new Error('深度優先搜索應該成功');
	...
	// 測試 2: 深度優先搜索與其他搜索策略比較
	test('深度優先搜索與其他搜索策略比較', async () => {
		const engine = new SeatAssignmentEngine({
			timeout: 10000,
			logLevel: 'WARN'
		});
	...
	test('深度優先搜索深度追蹤', async () => {
		...
		// 檢查深度追蹤功能
		const depthInfo = engine.trackDepth();
		if (!depthInfo) throw new Error('應該有深度信息');
		if (depthInfo.currentDepth === undefined) throw new Error('應該有當前深度信息');
		if (depthInfo.maxDepth === undefined) throw new Error('應該有最大深度信息');
	});

	// 測試 5: 深度優先搜索超時處理
	test('深度優先搜索超時處理', async () => {
